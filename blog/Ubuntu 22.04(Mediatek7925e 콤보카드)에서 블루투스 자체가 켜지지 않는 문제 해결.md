...
### 회고
지금까지는 GUI 기반의, 굉장히 하이레벨단에서 컴퓨터를 사용해와서 관심도 없고 잘 몰랐는데(애초에 맥북을 쓰면서 기본적인 블루투스 연결이 안되는 경험은 발생할 수 없는 일이었다만...) 이 컴퓨터가 어떻게 동작하는지 로우레벨의 이해하는 게 중요하다는 걸 깨달아서 좋다. 비록 이 노트북을 처음 개시한 날 거의 밤을 새가며 꾸부정한 자세로 문제와 다투느라 힘들었지만... 암튼 좋은 경험. 
